...
def test_aggregate_two(collection):


    message_data = MessageData(dt_from=datetime.strptime("2022-10-01T00:00:00", "%Y-%m-%dT%H:%M:%S"),
                            dt_upto=datetime.strptime("2022-11-30T23:59:00", "%Y-%m-%dT%H:%M:%S"),
                            group_type='day')
    result = aggregate(message_data, collection)
    assert result == {"dataset": [0, 0, 0, 195028, 190610, 193448, 203057, 208605, 191361,
                                  186224, 181561, 195264, 213854, 194070,208372, 184966,
                                  196745, 185221, 196197, 200647, 196755, 221695, 189114, 204853, 194652, 188096, 215141,
...
def test_aggregate_three(collection):
    message_data = MessageData(dt_from=datetime.strptime("2022-02-01T00:00:00", "%Y-%m-%dT%H:%M:%S"),
                            dt_upto=datetime.strptime("2022-02-02T00:00:00", "%Y-%m-%dT%H:%M:%S"),
                            group_type='hour')
    result = aggregate(message_data, collection)
    assert result == {"dataset": [8177, 8407, 4868, 7706, 8353,
                                  7143, 6062, 11800, 4077, 8820,
                                  4788, 11045, 13048, 2729, 4038,
                                  9888, 7490, 11644, 11232, 12177,
                                  2741, 5341, 8730, 4718, 0],
...
if __name__ == "__main__":
    ...
    test_aggregate_two(collection)
    test_aggregate_three(collection)